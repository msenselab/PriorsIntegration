...
##  run Baseline model
```{r}
baselinePath = paste0(rstanmodelPath, '/models/Baseline_linear')
runModelBaseline<- function(AllExpData){
  expList <- unique(AllExpData$Exp)
  rltall <- list()
  myBaselineModel <- stan_model(file="RStanCode/baseline_linear.stan") # Create model object (generates and compiles C++ code)
  for (expName in expList) {
    ...
    for (subNo in subList) {
      rlt = funFitStanBaseline_linear(sub_exp[[subNo]], myBaselineModel)
      saveRDS(rlt, file = paste0(baselinePath, "/rlt_Base_linear_",expName,"_", subNo,".rds"))
      rltall <- list.append(rltall, rlt)
    }
  }
  saveRDS(rltall, file = paste0(baselinePath, "/rlt_Baseline_linear.rds"))
}
...
funBaselineRlt <- function(path){
  ...
  NewY_mix_list <- {}
  modelResultAll <- list()
  bl_rlt = readRDS(paste0(getwd(), "/", path, "/rlt_Baseline_linear.rds"))
...
## models for IR session data

#### run models locally

```{r, message=FALSE, warning=FALSE, include=FALSE}
#source('RStanCode/rstan_linear.R')  
modelPath = paste0(rstanmodelPath, '/models/linear')

# function to run models on local computer
modellist <- c('LGM','DIM', 'PIM', 'IP')#'Baseline' 'BPM', 
runModelonlocal<- function(AllExpData, model){
  expList <- unique(AllExpData$Exp)
  rltall <-list()
  myrstanModel = NULL
  myrstanModel <- rstan::stan_model(file=paste0("RStanCode/", model, "_linear.stan")) 
  for (expName in expList) {
    data_exp <- AllExpData %>% dplyr::filter(valid == 1 & Exp == expName)
    ...
    for (subNo in subList) {
      ...
      rlt = funFitStanSub(subdata_mix, myrstanModel, 'all', 'linear')
      saveRDS(rlt, file = paste0(modelPath, "/rlt/rlt_",subNo,"_", expName,"_", model,".rds"))
      rltall <- list.append(rltall, rlt)
    }
  }
  saveRDS(rltall, file = paste0(modelPath, "/rlt/rlt_", model,".rds"))
}
...
#### fit first half data and second half data separately

```{r, message=FALSE, warning=FALSE, include=FALSE}
#source('RStanCode/rstan_linear_2part.R')  
...
runModelonlocal_2part<- function(AllExpData, model){
  ...
  rltall <-list()
  myrstanModel = NULL
  myrstanModel <- rstan::rstan::stan_model(file=paste0("RStanCode/", model, "_linear.stan")) 
  for (expName in expList) {
    data_exp <- AllExpData %>% dplyr::filter(valid == 1 & Exp == expName)
    data_exp$model <- model
    sub_exp <-  split(data_exp, data_exp$NSub) 
    ...
    for (subNo in subList) {
      ...
      rlt1 = funFitStanSub(subdata_mix%>%filter(id < total/2), myrstanModel, 'part1','linear')
      rlt2 = funFitStanSub(subdata_mix%>%filter(id >= total/2), myrstanModel, 'part2', 'linear')
      saveRDS(rlt1, file = paste0( modelPath, "/rlt/rlt_",subNo,"_", expName,"_", model,"_p1.rds"))
      saveRDS(rlt2, file = paste0(modelPath, "/rlt/rlt_",subNo,"_", expName,"_", model,"_p2.rds"))
      rltall <- list.append(rltall, rlt1)
      rltall <- list.append(rltall, rlt2)
    }
  }
  saveRDS(rltall, file = paste0(modelPath, "/rlt/rlt_", model,"_2part.rds"))
}
...
###  fit half data parallely
```{r}
runModelParallel_2part<- function(AllExpData, model){
  expList <- unique(AllExpData$Exp)
  rltall <-list()
  sub_exp_dat <- list()
   for (expName in expList) {
    ...
    for (subNo in subList) {
      ...
      # rlt1 = funFitStanSub(subdata_mix%>%filter(id < total/2), myrstanModel, 'part1','linear')
      # rlt2 = funFitStanSub(subdata_mix%>%filter(id >= total/2), myrstanModel, 'part2', 'linear')
      sub_exp_dat = list.append(sub_exp_dat, subdata_mix%>%filter(id < total/2))
  
      #rlt2 = runModelcluster(subdata_mix%>%filter(id >= total/2))
  ...
  rlt1 <- runModelcluster(sub_exp_dat)
  saveRDS(rlt1, file = paste0(getwd(), "/", modelPath, "/rlt/rlt_", model,"_part1.rds"))
}
...
runModelcluster<- function(sub_exp_dat){
  ...
  clusterExport(cl, c('funFitStanSub'))
  t0 = proc.time()
  resultlist <- clusterMap(cl, funFitStanSub, sub_exp_dat)
  stopCluster(cl)
  print(proc.time()-t0)
  return(resultlist)
}
...
# #execute the model
# for(model in modellist){
#   runModelParallel_2part(AllExpData, model)
# }
```
...
#### run models locally in one step
...
#### fit model locally in two steps
