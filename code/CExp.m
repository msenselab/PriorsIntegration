...
%constructor: two ways to initialize: 
    ...
    methods
        ...
        function trials=genTrials(obj,withinblock_repetitions, withinblock_factors, betweenblock_repetitions, betweenblock_factors)
         ...
         function  [subInfo] = subjectInfoBox(obj) 
            try
                promptParameters            = {'Subject Name', 'Age', 'Gender (F or M)',...
                    'First: S(1) or L(2)', 'Loc: S(1) or L(2)'};
                defaultParameters           = {'SubName', '20','F','1','1',};
                inputInfo                   = inputdlg(promptParameters, 'subjectInfo ', 1, defaultParameters); 
                ...
                obj.sName                   = char(inputInfo(1)); %subject information
                obj.sAge                    = char(inputInfo(2));
                obj.sGender                 = char(inputInfo(3));
                subInfo.sFirst              = str2num(inputInfo{4}); 
                subInfo.sLoc                = str2num(inputInfo{5});
                obj.aData = [subInfo.sFirst, subInfo.sLoc];
                obj.sName   = strcat(obj.sName, num2str(obj.sAge), obj.sGender, num2str(subInfo.sFirst), subInfo.sLoc);
                obj.sName = obj.sName(1:end-1);
        ...
        function obj = subInfo(obj,sp1,sp2)
            ...
            obj.sName = sub{1};
            obj.sName(find(isspace(obj.sName))) = [];
