...
modelResultPath = paste0(rstanmodelPath, '/Baseline2')
```
...
//real  prior_s[n_s]; // mean of internal prior of short group
//real  prior_l[n_l]; // mean of internal prior of long group
...
for (i in 1:n_s)
{
    w_s[i] = (mu_s[i]^2 * p_wf^2)/(mu_s[i]^2 * p_wf^2+ X_s[i]^2*wf^2);    // weight of current stimuls 
    Y_s[i] ~ normal(mu_s[i] *  w_s[i]+ (1- w_s[i])* X_s[i], sig_m_square + (mu_s[i]^2 * p_wf^2 * X_s[i]^2*wf^2) / mu_s[i]^2 * p_wf^2 + X_s[i]^2*wf^2); 
}

//long groups
for (i in 1:n_l)
{
    w_l[i] = (mu_l[i]^2 * p_wf^2)/(mu_l[i]^2 * p_wf^2+ X_l[i]^2*wf^2);  // weight of current stimuls 
    Y_l[i] ~ normal(mu_l[i] *  w_l[i]+ (1- w_l[i])* X_l[i],  
    sig_m_square + (mu_l[i]^2 * p_wf^2 * X_l[i]^2*wf^2) / mu_l[i]^2 * p_wf^2 + X_l[i]^2*wf^2);
...
real  w_new_mix[n_mix];   //  weight of stimuli in mix group
  ...
  { 
    mu_s_new[i] = normal_rng(xmean[1], p_wf^2 * xmean[1]^2);  // mean prior of short group 
    w_new_s[i] = (mu_s_new[i]^2 * p_wf^2)/(mu_s_new[i]^2 * p_wf^2+ X_s[i]^2*wf^2);    // weight of current stimuls 
    ynew_s[i] = normal_rng(mu_s_new[i] *  w_new_s[i]+ (1- w_new_s[i])* X_s[i], sig_m_square + (mu_s_new[i]^2 * p_wf^2 * X_s[i]^2*wf^2) / mu_s_new[i]^2 * p_wf^2 + X_s[i]^2*wf^2);
  }
  
  for (i in 1:n_l)   //prediction of long group
  {
     mu_l_new[i] = normal_rng(xmean[2], p_wf^2 * xmean[2]^2);   // mean prior of long group
     w_new_l[i] = (mu_l_new[i]^2 * p_wf^2)/(mu_l_new[i]^2 * p_wf^2+ X_l[i]^2*wf^2);  // weight of current stimuls 
    ynew_l[i] = normal_rng(mu_l_new[i] *  w_new_l[i]+ (1- w_new_l[i])* X_l[i],  
    sig_m_square + (mu_l_new[i]^2 * p_wf^2 * X_l[i]^2*wf^2) / mu_l_new[i]^2 * p_wf^2 + X_l[i]^2*wf^2);
  }
  ...
  for (i in 1:n_mix)   //prediction of mix group
  {
    ...
    mu_mix_new[i] = normal_rng(xmean[2], p_wf^2 * xmean[2]^2);   // mean prior of long group
    w_new_mix[i] = (mu_mix_new[i]^2 * p_wf^2)/(mu_mix_new[i]^2 * p_wf^2+ X_mix[i]^2*wf^2);  // weight of current stimuls 
    ynew_mix[i] = normal_rng(mu_mix_new[i] *  w_new_mix[i]+ (1- w_new_mix[i])* X_mix[i],  
    sig_m_square + (mu_mix_new[i]^2 * p_wf^2 * X_mix[i]^2*wf^2) / mu_mix_new[i]^2 * p_wf^2 + X_mix[i]^2*wf^2);
      
    }
   else{
        mu_mix_new[i] = normal_rng(xmean[1], p_wf^2 * xmean[1]^2);  // mean prior of short group
         w_new_mix[i] = (mu_mix_new[i]^2 * p_wf^2)/(mu_mix_new[i]^2 * p_wf^2+ X_mix[i]^2*wf^2);    // weight of current stimuls 
    ynew_mix[i] = normal_rng(mu_mix_new[i] *  w_new_mix[i]+ (1- w_new_mix[i])* X_mix[i], sig_m_square + (mu_mix_new[i]^2 * p_wf^2 * X_mix[i]^2*wf^2) / mu_mix_new[i]^2 * p_wf^2 + X_mix[i]^2*wf^2);
    }
...
funFitBaseLineStan <- function(data, rstanModel, filename){
  ...
  for (expName in expList) {
    ...
    for (subNo in subList) {
      ...
      parameters <- c("p_wf","wf", "sig_m_square", "w_new_l", "w_new_s", "w_new_mix", "ynew_s","ynew_l", "ynew_mix")
...
if (runModels == TRUE){
  funFitBaseLineStan(AllExpData, stanmodeBaseline,'Baseline2')
}
...
```{r include=FALSE}
PredY_l_Baseline <- read_csv(paste0(modelResultPath, "/PredY_l_Baseline2.csv"))
PredY_s_Baseline <- read_csv(paste0(modelResultPath, "/PredY_s_Baseline2.csv"))
PredY_mix_Baseline <- read_csv(paste0(modelResultPath, "/PredY_mix_Baseline2.csv"))
...
m_Baypar <- dplyr::group_by(AllDat_Bayparlist, Exp, Nsub) %>%
  dplyr::summarize( m_sig_m_square = mean(sig_m_square), m_wf = mean(wf),
            m_p_wf = mean(p_wf)) +
  theme_new
m_Baypar
```
...
###  p_wf in models
```{r}
ggplot(m_Baypar, aes(x = Exp, y = m_wf, color = Nsub, fill = Nsub, group = Nsub)) + 
