...
funFitBayesianStanH2 <- function(data, rstanModel, filename){
  ...
  for (expName in expList) {
    ...
    for (subNo in subList) {
      ...
      parameters <- c("p_wf_s","wf_s", "p_wf_l","wf_l","sig_m_square", "p_wf_m", "wf_m", "sig_mix_square", "ynew_mix", "w1_new", "w2_new")
  ...
  for (expName in expList) {
    ...
    for (subNo in subList) {
      ...
      subfit <- sampling(rstanModel, stan_data, chains = 4, iter = 4000,
       ...
       parameters <- c("g_wf", "p_wf_s","wf_s", "p_wf_l","wf_l","sig_m_square", "p_wf_m", "wf_m", "sig_mix_square", "ynew_mix", "w1_new", "w2_new")
      fitpar <- summary(subfit, pars = parameters)$summary
...
### run H3
```{r include=FALSE}
##Model integration of local priors( In H4, firstly integration of local priors, then integration of the global prior)
if (runModels == TRUE){
