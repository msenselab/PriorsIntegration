...
modelResultPath = paste0(rstanmodelPath, '/Version4')
```
...
for (sub in 1:nsubjs) {
  ...
  {
    if(X_s[sub, i] > 0){
        Y_s[sub, i] ~ normal(a_s[sub] + b_s[sub] * X_s[sub, i], sigma); //short groups
    }
  }
}
...
for (sub in 1:nsubjs) {
  ...
  {
    if(X_l[sub, j] > 0){
        Y_l[sub, j] ~ normal(a_l[sub] + b_l[sub] * X_l[sub, j], sigma); //long groups
    }
  }
}
...
for (sub in 1:nsubjs) {
  for (m in 1:n_mix)  
  {
    if (X_mix[sub, m] > 1){
      ...
      if (X_mix[sub, m] < 1){
        p_mix[sub, m] = a_s[sub] + b_s[sub] * X_mix[sub, m];
      }else{
        p_mix[sub, m] = a_l[sub] + b_l[sub] * X_mix[sub, m];
      }
      ...
      sig_mix[sub, m] = mean(X_mix[sub, 1:m])*g_wf;  //global prior of up mixed block 
      ...
      Y_mix[sub, m] ~ normal(y_mix_mean[sub, m], sigma); 
...
for (sub in 1:nsubjs) {
  for (m in 1:n_mix_te)  //prediction of up mixed range
  {
    if (X_mix_te[sub, m] > 0){
          // integration of local priors firstly
      if (X_mix_te[sub, m] < 1){
        p_mix_new[sub, m] = a_s[sub] + b_s[sub] * X_mix_te[sub, m];
      }else{
        p_mix_new[sub, m] = a_l[sub] + b_l[sub] * X_mix_te[sub, m];
      }
      p_sig_mix_new[sub, m] = mean(X_mix[sub, 1:m])*g_wf;  //global prior of up mixed block  
      ...
      // integration of the global prior
      sig_mix_new[sub, m] = mean(X_mix_te[sub, 1:m])*g_wf;  //global prior of up mixed block 
...
for (sub in 1:nsubjs) {
  for (m in 1:n_mix)  //integration of local priors firstly
  {
    if(X_mix[sub, m] > 0){
      ...
      sig_mix[sub, m] = mean(X_mix[sub, 1:m])*g_wf;  //global prior of up mixed block  
      p_sig_mix[sub, m] = p_mix[sub, m] * p_wf;
      w_mix[sub, m] = sig_mix[sub, m]^2 / (sig_mix[sub, m]^2 + p_sig_mix[sub, m]^2);
      ...
      Y_mix[sub, m] ~ normal(pp_w[sub, m] * X_mix[sub, m] + (1-pp_w[sub, m])*pp_mix[sub, m], sigma); 
    }
  }
 }
}
...
for (sub in 1:nsubjs) {
  for (m in 1:n_mix_te)  //prediction of up mixed range
  {
    // first integration 
    if(X_mix_te[sub, m] < 1){
      p_mix_new[sub, m] = a_s[sub] + b_s[sub] * X_mix[sub, m];
    }
    else{
      p_mix_new[sub, m] = a_l[sub] + b_l[sub] * X_mix[sub,m];
    }
    sig_mix_new[sub, m] = mean(X_mix_te[sub, 1:m])*g_wf;  //global prior of up mixed block  
...
funFitBayesianStanV4 <- function(data, rstanModel, filename){
  ...
  for (expName in expList) {
    ...
    for(i in 1 : nsubjs){
     ...
     Baypar <- data.frame("a_s" = a_s, "b_s" = b_s, "a_l" = a_l, "b_l" = b_l)
     Baypar$Exp = expName
     Baypar$wf = wf
     Baypar$p_wf = p_wf
     Baypar$g_wf = g_wf
